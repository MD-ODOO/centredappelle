    ...
    def _compute_estimated_end(self):
        for record in self:
            if record.state not in ['close', 'cancel'] and record.has_total_amount and record.monthly_amount:
                ...
                months = ceil(record.remaining_amount / record.monthly_amount)

                # 🔥 Correction : fin du mois et non début
                estimated_date = record.date_start + relativedelta(months=months - 1)
                record.date_estimated_end = end_of(estimated_date, 'month')

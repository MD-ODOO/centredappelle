...
class HrPayslip(models.Model):
    ...
    def _compute_input_line_ids(self):
        ...
        for slip in self:
            ...
            # 1. Nettoyage complet des anciens inputs liés aux attachments
            attachment_type_ids = self.env['hr.payslip.input.type'].search([
            ...
            if not slip.employee_id or not slip.struct_id or not slip.date_to:
                slip.update({'input_line_ids': input_line_vals})
                continue

            # 3. Récupérer les attachments de l'employé
            ...
            # 4. Filtrer les attachments valides
            valid_attachments = attachments.filtered(
                lambda a:
                    a.date_start <= slip.date_to
                    and (not a.date_estimated_end or a.date_estimated_end >= slip.date_from)
                    and (
                        not a.other_input_type_id.struct_ids
                        or slip.struct_id.id in a.other_input_type_id.struct_ids.ids
                    )
            )

            # 5. Grouper par type d'input
            for input_type, atts in valid_attachments.grouped('other_input_type_id').items():
                amount = atts._get_active_amount()
                name = ', '.join(atts.mapped('description'))

                input_line_vals.append(Command.create({
                    'name': name,
                    'amount': amount if not slip.credit_note else -amount,
                    'input_type_id': input_type.id,
                    'code': input_type.code,  # ✅ IMPORTANT
                }))
    ...
    # def _onchange_force_inputs(self):
